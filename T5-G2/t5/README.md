...
At any time, the player may submit their solution to end the match. The outcome (win or loss) is determined based on their score and the selected game mode. Upon submission, the latest compilation attempt will be recompiled to also retrieve EvoSuite metrics via a request to the T8 service. If the compilation is successful, both the coverage and EvoSuite metrics will be obtained and evaluated alongside the corresponding results of the selected opponent.
