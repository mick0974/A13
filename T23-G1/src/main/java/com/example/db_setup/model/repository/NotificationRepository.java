...
public interface NotificationRepository extends JpaRepository<Notification, Long> {
    ...
    Page<Notification> findByPlayerID(long playerID, Pageable pageable);

    // Notifiche non lette per timestamp
    List<Notification> findByPlayerIDAndIsReadFalseOrderByTimestampDesc(Long playerID);
    ...
    void markAllAsReadByPlayerID(long playerID);
    ...
    Page<Notification> findByPlayerIDAndType(long playerID, String type, Pageable pageable);

    // Filtro per playerID, tipo di notifica e stato di lettura
    Page<Notification> findByPlayerIDAndTypeAndIsRead(long playerID, String type, boolean isRead, Pageable pageable);

    // Filtro per playerID e stato di lettura
    Page<Notification> findByPlayerIDAndIsRead(long playerID, boolean isRead, Pageable pageable);

    // Trova le notifiche per un utente che corrispondono a più tipi e stato di lettura
    Page<Notification> findByPlayerIDAndTypeInAndIsRead(long playerID, List<String> types, boolean isRead, Pageable pageable);
